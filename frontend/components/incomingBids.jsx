...
function incomingBids() {
  ...
  const getBids = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/product/bid/incomingbids`, { withCredentials: true })
      setBids(res.data)
    } catch (err) {
  ...
  const handleAction = async (bidId, action) => {
    ...
    try {
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/product/bid/${bidId}/${action}`,
        {},
