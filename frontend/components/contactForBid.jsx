...
function contactForBid() {
  ...
  const getOneBid = async () => {
    try {
      const res     = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/product/getOneBidData/${bidId}`,
        { withCredentials: true }
      )
      const bidData = res.data

      setProduct(bidData.productId)
      setBidAmount(bidData.bidAmount)

      // show the OTHER person
      if (bidData.seller._id === currentUserId) {
        setContact(bidData.buyer)
      } else {
        setContact(bidData.seller)
      }
    } catch (err) {
  ...
  /* ── helpers ── */
  const whatsappLink = (number, name, productName, amount) => {
    const cleaned = number?.replace(/\D/g, '')
    const msg = encodeURIComponent(
      `Hi ${name}! I'm reaching out regarding the bid of ₹${Number(amount).toLocaleString('en-IN')} on "${productName}" on CampusHub. Let's connect!`
    )
    return `https://wa.me/91${cleaned}?text=${msg}`
  }

  const gmailLink = (email, productName, amount) => {
    const subject = encodeURIComponent(`CampusHub — Bid of ₹${Number(amount).toLocaleString('en-IN')} on: ${productName}`)
    const body    = encodeURIComponent(
      `Hi,\n\nI'm following up on the bid of ₹${Number(amount).toLocaleString('en-IN')} on "${productName}" listed on CampusHub.\n\nLooking forward to connecting!\n\nRegards`
    )
  ...
  const S = {
    page: {
      ...
      background: '#0a0a0f',
      backgroundImage:
        'radial-gradient(ellipse 80% 50% at 50% -10%, rgba(99,102,241,0.14) 0%, transparent 65%),' +
    ...
    checkRing: {
      ...
      boxShadow: '0 0 40px rgba(99,102,241,0.20)',
    ...
    productInfo: { display: 'flex', flexDirection: 'column', gap: 3, flex: 1 },
    ...
    stripRight: { display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flexShrink: 0 },
    bidLabel: { fontSize: 10, color: '#475569', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' },
    bidAmountText: {
    ...
    contactCard: {
      ...
      backdropFilter: 'blur(20px)',
      boxShadow: '0 8px 48px rgba(0,0,0,0.45), 0 0 0 1px rgba(99,102,241,0.06)',
      marginBottom: 18,
    },

    cardHeader: {
      background: 'linear-gradient(135deg,rgba(99,102,241,0.13) 0%,rgba(139,92,246,0.10) 100%)',
    ...
    avatarRing: {
      ...
      padding: 3, boxShadow: '0 0 32px rgba(99,102,241,0.32)',
    },
    avatarInner: {
      width: '100%', height: '100%', borderRadius: '50%', background: '#0a0a0f',
      display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
    },
    avatarInitial: { fontFamily: "'Space Grotesk',sans-serif", fontSize: 30, fontWeight: 800, color: '#a5b4fc' },
    nameBlock: { textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 4 },
    roleBadge: {
      display: 'inline-flex', alignItems: 'center', gap: 6,
      padding: '4px 14px', borderRadius: 999,
      fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase',
      color: '#a5b4fc', background: 'rgba(99,102,241,0.14)', border: '1px solid rgba(99,102,241,0.28)',
    ...
    infoLabel: { fontSize: 10, color: '#475569', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 2 },
    infoValue: { fontFamily: "'Space Grotesk',sans-serif", fontSize: 14, fontWeight: 600, color: '#e2e8f0' },
  ...
  const icons = {
    email: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" strokeWidth="1.8" strokeLinecap="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>,
    phone: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" strokeWidth="1.8" strokeLinecap="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81 19.79 19.79 0 01.16 1.18 2 2 0 012.14.02h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006.63 6.63l1.06-1.06a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
    college: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" strokeWidth="1.8" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
    course: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" strokeWidth="1.8" strokeLinecap="round"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z"/><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z"/></svg>,
    year: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
    user: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" strokeWidth="1.8" strokeLinecap="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
  }
  ...
  if (error) return (
    <div style={S.page}>
      ...
      <div style={{ ...S.wrap, ...S.stateWrap }}>
        ...
        <button onClick={() => navigate(-1)} style={{ padding: '10px 22px', borderRadius: 10, border: '1px solid rgba(99,102,241,0.35)', background: 'rgba(99,102,241,0.08)', color: '#a5b4fc', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: "'Space Grotesk',sans-serif" }}>
          Go Back
        </button>
      </div>
    </div>
  ...
  const isSeller  = contact?._id !== currentUserId
      ...
      <div style={S.wrap}>
        ...
        <div style={S.heroWrap}>
          <div style={S.checkRing}>
            <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" strokeWidth="2.2" strokeLinecap="round">
              <polyline points="20 6 9 17 4 12"/>
            </svg>
          </div>
          <h1 style={S.heroTitle}>Bid Accepted!</h1>
        ...
        {product && (
          <div style={S.productStrip}>
            ...
            <div style={S.stripRight}>
              <span style={S.bidLabel}>Bid Amount</span>
              <span style={S.bidAmountText}>
                ₹{bidAmount != null ? Number(bidAmount).toLocaleString('en-IN') : '—'}
              </span>
            </div>
          </div>
        )}
        ...
        <div style={S.contactCard}>
          ...
          <div style={S.cardHeader}>
            ...
            <div style={S.nameBlock}>
              <span style={S.roleBadge}>
                <svg width="8" height="8" viewBox="0 0 24 24" fill="#a5b4fc"><circle cx="12" cy="12" r="10"/></svg>
                {roleLabel}
              </span>
              <span style={S.contactFullname}>{contact?.fullname || contact?.username}</span>
              {contact?.username && <span style={S.contactUsername}>@{contact.username}</span>}
            </div>
          </div>
          ...
          <div style={S.infoSection}>
            {infoRows.map((row, i) => (
              <div key={i} style={{ ...S.infoRow, borderBottom: i === infoRows.length - 1 ? 'none' : '1px solid rgba(99,102,241,0.07)' }}>
          ...
          <div style={S.actionsSection}>
            ...
            {contact?.contactNumber && (
              <a
                href={whatsappLink(contact.contactNumber, contact.fullname || contact.username, product?.productName || 'the product', bidAmount)}
            ...
            {contact?.email && (
              <a
                href={gmailLink(contact.email, product?.productName || 'your listing', bidAmount)}
                target="_blank" rel="noopener noreferrer"
                style={S.btnGmail}
