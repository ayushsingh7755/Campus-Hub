...
function services() {
    ...
    const[iszeroServices,setzeroService]=useState(false)
  ...
  return (
    ...
    {iszeroServices? (<div className='text-center'> No serivces registered for this college</div> ):(
        <>
    ...
    showOwner >= 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
            <div className="relative w-full max-w-sm sm:max-w-md md:max-w-lg bg-zinc-900 border border-purple-600 rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">

                {/* Close button - always top-right, doesn't interfere with layout */}
                <button
                    onClick={() => setshowOwner(-1)}
                    className="absolute top-3 right-3 cursor-pointer text-white/70 hover:text-white transition-colors"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" className="sm:w-6 sm:h-6" color="currentColor" fill="none">
                ...
                <img
                    className="h-20 w-20 sm:h-[140px] sm:w-[140px] md:h-[200px] md:w-[200px] rounded-full flex-shrink-0 border border-violet-500/30 shadow-[0_0_25px_rgba(168,85,247,0.25)] mt-4 sm:mt-0"
                    src={cardData[showOwner].productOwner.avatar}
                    alt="No Profile Image"
                />

                <div className="text-[rgb(91,158,229)] font-normal tracking-[-0.5px] font-['Outfit'] text-center sm:text-left space-y-1 sm:mt-8 break-words">
                    <h3 className="text-sm sm:text-base md:text-lg">Name: {cardData[showOwner].productOwner.fullname}</h3>
                    <p className="text-sm sm:text-base md:text-lg">Username: {cardData[showOwner].productOwner.username}</p>
                    <p className="text-sm sm:text-base md:text-lg break-all">Email: {cardData[showOwner].productOwner.email}</p>
                </div>
